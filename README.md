...
| **Post-It (포스트잇)** | 메모를 기록할 수 있는 투명한 텍스트 편집기. |
| **Clock (시계)** | 현재 시스템 시간을 표시하는 작은 디지털 시계. |
| **Cookie (포춘 쿠키)** | **클릭 시** 쿠키가 열리며, 내부에 숨겨진 무작위 **명언이나 문구**가 나타납니다. |
| **Asparagus (아스파라거스)** | 거북이(Turtle) 위젯의 **주요 먹이** 아이템. |
| **Turtle (거북이)** | **배고픔** 상태를 가지며, 일정 시간 아스파라거스를 먹지 않으면 사망할 수 있는 펫 위젯입니다. |
| **Ball (공)** | 터틀 위젯에게 **위험한 아이템**입니다. |
| **Cloud (구름)** | 단순히 떠다니며 데스크탑 분위기를 조성하는 위젯입니다. |
| **Black Hole (블랙홀)** | 다른 위젯들을 흡수하여 화면을 정리하는 **'휴지통'** 역할의 위젯. |
